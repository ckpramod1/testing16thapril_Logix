...
{
    public partial class BL4MArpt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString.ToString().Contains("blno"))
            {
                ...
                DataTable dtsign = masterObj.Getblformtrpt(Convert.ToInt32(Session["LoginDivisionId"].ToString()));
                if (dtsign.Rows.Count > 0)
                {
                    byte[] sealimageBytes = ((byte[])dtsign.Rows[0]["blseal"]);
                    string base64Stringseal = Convert.ToBase64String(sealimageBytes);
                    img_seal.ImageUrl = "data:image/png;base64," + base64Stringseal;
                    byte[] signimageBytes = ((byte[])dtsign.Rows[0]["blsign"]);
                    string base64Stringsign = Convert.ToBase64String(signimageBytes);
                    img_signature.ImageUrl = "data:image/png;base64," + base64Stringsign;
                }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    lbl_jobno.Text ="JOB # "+ dt.Rows[0]["jobno"].ToString();
                    lbl_etd.Text = Convert.ToDateTime(dt.Rows[0]["bldate"]).ToString("dd/MM/yyyy");
                    lbl_conaddress.Text = dt.Rows[0]["caddress"].ToString();
                    Label2.Text = dt.Rows[0]["dportname"].ToString();
                    ...
                    Label10.Text = dt.Rows[0]["freightCollect"].ToString();
                    ...
                    if (bltype == "D")
                    {
                        ...
                        lbl_sonboard.Visible = false;
                        string script = @"<script>
                        var elements = document.getElementsByClassName('lblsob');
                    ...
                    if (bltype == "O" || bltype == "OS")
                    {
                        ...
                        {
                            lbl_bltype.Text = "SEAWAY BL";
                            img_signature.Visible = true;
                            img_seal.Visible = true;
                            lbl_orl.Text = "NON NEGOTIABLE";
                        }
