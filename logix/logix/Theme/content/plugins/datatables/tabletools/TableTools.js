...
TableTools = function( oDT, oOpts )
{
	...
	{
		alertify.alert( "Warning: TableTools must be initialised with the keyword 'new'" );
	}
	...
	"_fnButtonDefinations": function ( buttonSet, wrapper )
	{
		...
		for ( var i=0, iLen=buttonSet.length ; i<iLen ; i++ )
		{
			if ( typeof buttonSet[i] == "string" )
			{
				...
				{
					alertify.alert( "TableTools: Warning - unknown button type: "+buttonSet[i] );
					continue;
				}
			...
			else
			{
				...
				{
					alertify.alert( "TableTools: Warning - unknown button type: "+buttonSet[i].sExtends );
					continue;
				}
	...
	"ajax": $.extend( {}, TableTools.buttonBase, {
		...
		"fnClick": function( nButton, oConfig ) {
			...
			$.ajax( {
				...
				"cache": false,
				"error": function () {
					alertify.alert( "Error detected when sending table data to server" );
				}
			} );
		},
		"fnAjaxComplete": function( json ) {
			alertify.alert( 'Ajax complete' );
		}
	} ),
...
{
	alertify.alert( "Warning: TableTools 2 requires DataTables 1.9.0 or newer - www.datatables.net/download");
}
